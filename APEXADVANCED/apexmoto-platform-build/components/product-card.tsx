...
export function ProductCard({ product, index }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="relative bg-white/5 rounded-[2rem] border border-white/10 overflow-hidden backdrop-blur-sm transition-all duration-500 group-hover:border-apex-orange/40 group-hover:shadow-[0_0_60px_rgba(255,77,0,0.18),0_0_120px_rgba(255,77,0,0.06)]">
        {/* Image Section */}
        <div className="relative aspect-[4/5] overflow-hidden bg-[#0a0a0a]">
          ...
          <motion.div
            ...
            style={{
              background: "repeating-linear-gradient(0deg, transparent, transparent 3px, rgba(255,77,0,0.04) 3px, rgba(255,77,0,0.04) 4px)",
            }}
          />

          {/* Sweeping orange light beam */}
          <motion.div
          ...
          >
            <div className="w-1/3 h-full bg-gradient-to-r from-transparent via-apex-orange/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          </motion.div>

          {/* Quick Actions Overlay */}
          <div className="absolute inset-0 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-t from-black/80 to-transparent z-20">
            <motion.div whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }}>
              <Button 
                size="icon" 
                variant="outline"
                className="w-14 h-14 rounded-2xl bg-black/40 border-apex-orange/30 text-white hover:bg-apex-orange hover:border-apex-orange hover:text-white backdrop-blur-md shadow-[0_0_20px_rgba(255,77,0,0.2)] transition-all duration-200"
        ...
        <div className="p-6 space-y-4">
          ...
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>
            <Button 
              className={`w-full h-12 rounded-2xl font-black italic text-[10px] tracking-widest uppercase gap-3 relative overflow-hidden transition-all duration-300 ${
                product.stockStatus === "Backorder" 
                ? "bg-white/5 border border-white/10 text-neutral-500 cursor-not-allowed" 
                : "bg-white text-black hover:bg-apex-orange hover:text-white hover:shadow-[0_0_30px_rgba(255,77,0,0.4)]"
              }`}
